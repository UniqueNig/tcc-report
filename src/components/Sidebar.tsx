...
import { X, FileText, LayoutDashboard, LogOut, Users, Building2, ClipboardList, ShieldCheck } from "lucide-react";
...
export interface SidebarUser {
  name: string;
  unit?: string;  // shown for UNIT_HEAD
  role: UserRole;
}
...
const NAV_ITEMS: Record<UserRole, NavItem[]> = {
  ...
  ADMIN: [
    ...
    { label: "Manage Units",  href: "/dashboard/admin/units",        icon: Building2 },
  ],
};
      ...
      >
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
          ...
          {navItems.map((item) => {
            ...
            const isActive =
              pathname === item.href ||
              (item.href !== `/dashboard/${user.role.toLowerCase().replace("_", "-")}` &&
        ...
        <div className="px-3 py-4 border-t border-stone-100 dark:border-neutral-800 shrink-0">
          <div className="flex items-center gap-3 px-2 py-2 rounded-xl">
            ...
            <button
              onClick={() => {
                // Wire to: signOut() from next-auth, or clear JWT and redirect to /login
              }}
